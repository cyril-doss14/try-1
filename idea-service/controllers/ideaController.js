...
exports.toggleCollaboration = async (req, res) => {
  try {
    ...
    // Call auth-service to update collaborationWishes
    const authServiceUrl = 'http://auth-service:5000/api/auth';

    if (isCollaborating) {
      idea.collaborators = idea.collaborators.filter(id => id.toString() !== userId);

      // ❌ Remove collaboration wish from idea owner
      await axios.put(`${authServiceUrl}/remove-collab-wish`, {
        userId: ideaOwnerId,
        wishUserId: userId
      });
    } else {
      ...
      await axios.put(`${authServiceUrl}/add-collab-wish`, {
        userId: ideaOwnerId,
        wishUserId: userId
      });
    }
